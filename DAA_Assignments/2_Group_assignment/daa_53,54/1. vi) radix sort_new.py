...
def counting_sort(arr, exp, comparisons, basic_operations):
    ...
    for i in range(n):
        index = arr[i] // exp  #index for position of desired digit,according to one's or 10's place
        count[index % 10] += 1  #shows number of elements having same digit in that index
        basic_operations[0] += 1  # Increment basic operations count (for the index calculation)

    for i in range(1, 10):
        count[i] += count[i - 1]  # counting elements in order to make sure correct positions of elements
        basic_operations[0] += 1  # Increment basic operations count (for count array updates)

    i = n - 1
    while i >= 0:
        index = arr[i] // exp
        output[count[index % 10] - 1] = arr[i]  #arr[i] is placed in the correct position in the output array based on the count of occurrences of the current digit.keeping the order of elements with equal digits
        count[index % 10] -= 1  #one occurrence of that digit has been correctly placed
        i -= 1
        basic_operations[0] += 3  # Increment basic operations count (for assignments, decrements, and comparisons)

    for i in range(n):
        arr[i] = output[i]  #avoid this to get in-place
